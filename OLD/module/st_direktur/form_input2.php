<?php
$id = $_GET['id'];
$query = "select * from surat_tugas_direktur where id_st ='$id'";
...
?>
    <section class="content">
        ...
        <div class="row">
          ...
          <div class="col-md-12">
			...
			<table class="col-md-12 form-control-sm">
				...
				<tr><td>Tanggal</td>
					<td><input type="date" class="col-md-8 form-control date-picker form-control-sm" name="tglst" value="<?php echo $data['tanggal']; ?>" readonly></td>
				</tr>
				<tr><td>Tempat</td>
				...
				<tr><td>Waktu Kegiatan Selesai</td>
					<td><input type="text" class="col-md-8 form-control form-control-sm" value="<?php echo $data['akhir']; ?>" readonly></td>
				</tr>
				<tr><td>Sifat</td>
					<td><input type="text" class="col-md-8 form-control form-control-sm" value="<?php echo $data['sifat']; ?>" readonly></td>
				</tr>
			</table>
				<br>
            ...
            <div class="form-group">
			  <table id="example2" class="table table-bordered table-hover form-control-sm">
					<th>No.</th><th>Nama </th><th>NIP</th><th>Aksi</th>	
						<?php
							$query2 = "select x.id_peg id_pegawai, x.nip18, x.nama, x.id_sie, y.* from pegawai x left join (SELECT a.id_pegst_direktur id_tugas, a.id_peg, b.id_st_dir, nomor, tanggal, awal, akhir, tempat, agenda FROM pegawai_tugas_direktur a, surat_tugas_direktur b where a.id_st_dir = b.id_st and a.id_st_dir='$id') y on x.id_peg = y.id_peg where nip18 !='' order by id_tugas desc, id_sie, nama";
							$run2 = mysqli_query($konek,$query2);
							$no = 1;
					while($data2=mysqli_fetch_array($run2))
					{
				?>				
				<form role="form" action="module/st_direktur/proses_input3.php" method="post">
					<tr><td>
						<input type="text" name="idpeg" value="<?php echo $data2['id_pegawai']; ?>">
						<input type="text" name="idst" value="<?php echo $data['id_st']; ?>">
						<input type="text" name="nama" value="<?php echo $data2['nama']; ?>">
						<input type="text" name="nip18" value="<?php echo $data2['nip18']; ?>">
						<input type="text" name="seksi" value="<?php echo $data2['id_sie']; ?>">
						<?php echo $no; ?></td>
	 ...
	 <?php
		...
		<div class="card card-primary">
			<div class="card-header">
                <h3 class="card-title">Form input ST Pegawai </h3>
              </div>
			  <div class="card-body ">
              <!-- /.card-header -->
              <!-- form start -->
			...
			<table class="col-md-12 form-control-sm">
				...
				<br>
				  <div class="text-center">
					...
					<a href="?module=surat_tugas_direktur" class="btn btn-sm btn-danger">
                      Batal
                    </a>
                  </div>
			</form>
