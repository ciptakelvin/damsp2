...
<?php
$id_st = $_GET['id'];
$query = "select *from surat_tugas where id_st='$id_st'";
          ...
          <div class="col-md-12">
            ...
            <div class="card card-primary">
              <div class="card-header">
                <h3 class="card-title"><h3 class="card-title">Surat Tugas Pegawai Subdit</h3>
              </div>
			  <div class="card-body">
              ...
              <form role="form" action="module/st/proses_edit.php" method="post">
			  ...
			  <input type="hidden" name ="idst" value="<?php echo $id_st; ?>">
				...
				<tr><td>Tanggal</td><td><input value="<?php echo $data['tanggal']; ?>" class="col-md-8 form-control form-control-sm" type="text" name="tanggal"></td><td></td></tr>
				...
				<tr><td>Waktu Kegiatan</td><td><input value="<?php echo $data['awal']; ?>" class="col-md-6 form-control form-control-sm" type="text" name="waktu"></td></tr>
			  </table>
			  <br>
			  ...
			  <table id="example2" class="table table-bordered table-hover form-control-sm">
					<th>No.</th><th>Nama </th><th>NIP</th><th>Aksi</th>	
						<?php
							$query2 = "select x.id_peg id_pegawai, x.nip18, x.nama, x.id_sie, y.* from pegawai x left join (SELECT a.id_pegst id_tugas, a.id_peg, b.id_st, nomor, tanggal, awal, akhir, tempat, agenda FROM pegawai_tugas a, surat_tugas b where a.id_st = b.id_st and a.id_st = '$id_st') y on x.id_peg = y.id_peg where nip18 !='' order by id_tugas desc, id_sie, nama";
							$run2 = mysqli_query($konek,$query2);
							$no = 1;
					while($data2=mysqli_fetch_array($run2))
					{
				?>				
				<form role="form" action="module/st/proses_input4.php" method="post">
					<tr><td>
				...
				<?php
					...
					{
				?>
						<a title="Hapus" href="?module=del_peg&idst=<?php echo $data['id_st'];?>&idtgs=<?php echo $data2['id_tugas']; ?>"><button type="submit" title="Hapus" class="btn-danger" onclick="myFunction_1()" ><img src="img/del.png" height="20" width="20"></a>
				<?php
					}
					else
					{
