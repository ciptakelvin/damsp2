     ...
     <div class="container-fluid">
        <div class="row">
          ...
          <td><input class="col-md-8 form-control form-control-sm" type="date" value="<?php echo $data['tanggal_surat']; ?>" readonly></td>
			...
			<?php
			include("conn.php");
			$kueritim = "select * from pegawai_events where id_events_pgw = '$idst' and status = '1'";
			$runtim = mysqli_query($konek,$kueritim);
			$urut = 1;
			while($datatim=mysqli_fetch_array($runtim))
			{
			?>
			<tr><td>Pegawai <?php echo $urut; ?></td><td><input class="col-md-8 form-control form-control-sm" type="text" value="<?php echo $datatim['nama']; ?>" readonly></td></tr>
			<?php
			$urut++;
			}
			?>
			...
			<form role="form" action="module/penugasan_pegawai_st/proses_input_tim.php" method="post">
				<input type="hidden" name="id_st" value="<?php echo $idst; ?>">
				<tr><td>Pegawai</td>
				  <td><select class="col-md-6 form-control form-control-sm select2bs4" name="anggota" id="nama" onchange="populate()">
						<option value=''></option>
						<?php
						include("conn.php");
                        $idst = $_GET['id'];
